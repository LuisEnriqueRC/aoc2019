...
function intCodeComputer(program) {
    ...
    for (let i = 0; i < program.length; i+=4) {
        ...
        let instruction = parameters[0];
        instruction = instruction.toString().padStart(4, '0');
        let opcode = 0;;
        let paramMode1 = 0, paramMode2 = 0;
        let param1 = program[parameters[1]], param2 = program[parameters[2]];
        if (instruction.length > 2) {
            opcode = Number(instruction.slice(-2));
            paramMode1 = Number(instruction[1]);
            paramMode2 = Number(instruction[0]);
            param1 = paramMode1 == 0 ? program[parameters[1]] : parameters[1];
            param2 = paramMode2 == 0 ? program[parameters[2]] : parameters[2];
        }
        if (opcode == 1) { // suma
            ...
            let input = 1;
            program[parameters[1]] = input;
            i -= 2;
        } else if (opcode == 4) {
            let output = program[parameters[1]];
            outputs.push(output);
            i -= 2;
        } else if (opcode == 99) {
